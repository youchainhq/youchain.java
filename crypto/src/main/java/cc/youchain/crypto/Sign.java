...
import cc.youchain.utils.Assertions;
...
public class Sign {
    ...
    static final String MESSAGE_PREFIX = "\u0019YOUChain Signed Message:\n";

    static byte[] getYOUChainMessagePrefix(int messageLength) {
        return MESSAGE_PREFIX.concat(String.valueOf(messageLength)).getBytes();
    }

    static byte[] getYOUChainMessageHash(byte[] message) {
        byte[] prefix = getYOUChainMessagePrefix(message.length);
    ...
    public static SignatureData signPrefixedMessage(byte[] message, ECKeyPair keyPair) {
        return signMessage(getYOUChainMessageHash(message), keyPair, false);
    }
    ...
    /**
     * Given an arbitrary piece of text and an YOUChain message signature encoded in bytes,
    ...
    /**
     * Given an arbitrary message and an YOUChain message signature encoded in bytes,
    ...
    public static BigInteger signedPrefixedMessageToKey(
            byte[] message, SignatureData signatureData) throws SignatureException {
        return signedMessageHashToKey(getYOUChainMessageHash(message), signatureData);
    }
