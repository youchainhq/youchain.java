...
import cc.youchain.abi.datatypes.generated.AbiTypes;
...
import cc.youchain.protocol.core.methods.response.AbiDefinition;
...
public class SolidityFunctionWrapper extends Generator {
    ...
    private static final String YOUCHAIN = "youChain";
    private static final String CREDENTIALS = "credentials";
    private static final String CONTRACT_GAS_PROVIDER = "contractGasProvider";
    private static final String TRANSACTION_MANAGER = "transactionManager";
    private static final String INITIAL_VALUE = "initialLuValue";
    ...
    private static final String LU_VALUE = "luValue";
    ...
    private TypeSpec.Builder createClassBuilder(String className, String binary) {
        ...
        String javadoc = CODEGEN_WARNING + getYOUChainVersion();
    ...
    private String getYOUChainVersion() {
        String version;

        try {
            // This only works if run as part of the YOUChain command line tools which contains
            // a version.properties file
            version = Version.getVersion();
        } catch (IOException | NullPointerException e) {
            version = Version.DEFAULT;
        }
        return "\n<p>Generated with YOUChain version " + version + ".\n";
    }
    ...
    private static MethodSpec buildDeployWithParams(
        ...
        if (isPayable && !withGasProvider) {
            methodBuilder.addStatement(
                    ...
                    className, YOUCHAIN, authName, GAS_PRICE, GAS_LIMIT, BINARY, INITIAL_VALUE);
            methodBuilder.addAnnotation(Deprecated.class);
        } else if (isPayable && withGasProvider) {
            methodBuilder.addStatement(
                    "return deployRemoteCall("
                            + "$L.class, $L, $L, $L, $L, encodedConstructor, $L)",
                    className, YOUCHAIN, authName, CONTRACT_GAS_PROVIDER, BINARY, INITIAL_VALUE);
        } else if (!isPayable && !withGasProvider) {
            methodBuilder.addStatement(
                    "return deployRemoteCall($L.class, $L, $L, $L, $L, $L, encodedConstructor)",
                    className, YOUCHAIN, authName, GAS_PRICE, GAS_LIMIT, BINARY);
            methodBuilder.addAnnotation(Deprecated.class);
        } else {
            methodBuilder.addStatement(
                    "return deployRemoteCall($L.class, $L, $L, $L, $L, encodedConstructor)",
                    className, YOUCHAIN, authName, CONTRACT_GAS_PROVIDER, BINARY);
        }
        ...
        if (withGasProvider) {
            toReturn.addParameter(ContractGasProvider.class, CONTRACT_GAS_PROVIDER)
                    .addStatement("return new $L($L, $L, $L, $L)", className,
                            CONTRACT_ADDRESS, YOUCHAIN, authName, CONTRACT_GAS_PROVIDER);
        } else {
            toReturn.addParameter(BigInteger.class, GAS_PRICE)
                    .addParameter(BigInteger.class, GAS_LIMIT)
                    .addStatement("return new $L($L, $L, $L, $L, $L)", className,
                            CONTRACT_ADDRESS, YOUCHAIN, authName, GAS_PRICE, GAS_LIMIT)
                    .addAnnotation(Deprecated.class);
        }
    ...
    private void buildTransactionFunction(
        ...
        if (functionDefinition.isPayable()) {
            methodBuilder.addParameter(BigInteger.class, LU_VALUE);
        }
        ...
        if (functionDefinition.isPayable()) {
            methodBuilder.addStatement(
                    "return executeRemoteCallTransaction(function, $N)", LU_VALUE);
        } else {
    ...
    MethodSpec buildEventFlowableFunction(
        ...
        MethodSpec.Builder flowableMethodBuilder =
                MethodSpec.methodBuilder(generatedFunctionName)
                        .addModifiers(Modifier.PUBLIC)
                        .addParameter(YOUFilter.class, FILTER)
        ...
        flowableMethodBuilder
                .addStatement("return youChain.youLogFlowable(filter).map($L)", converter);
    ...
    MethodSpec buildDefaultEventFlowableFunction(
        ...
        flowableMethodBuilder.addStatement("$1T filter = new $1T($2L, $3L, "
                + "getContractAddress())", YOUFilter.class, START_BLOCK, END_BLOCK)
    ...
    private static Class<?> getStaticArrayTypeReferenceClass(String type) {
        try {
            return Class.forName("cc.youchain.abi.datatypes.generated.StaticArray" + type);
